...
export function CheckoutPage() {
  ...
  useEffect(() => {
    api.get('/time-slots/available', { params: { date, type: fulfillment } })
      .then(({ data }) => {
        setSlots(data.slots)
  ...
  useEffect(() => {
    if (fulfillment === 'delivery' && address.length > 5) {
      api.post('/delivery/calculate', { address, area })
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.guest_token) {
        localStorage.setItem(`guest_${data.order.order_number}`, data.guest_token)
      }
      clearCart()
      navigate(`/order-success/${data.order.order_number}`, {
        state: { order: data.order, guestToken: data.guest_token },
      })
    } catch (err: unknown) {
      ...
      <form onSubmit={submit} className="mt-8 grid gap-8 lg:grid-cols-5">
        <div className="space-y-6 lg:col-span-3">
            ...
            <CardContent className="space-y-4">
              ...
              {fulfillment === 'delivery' && (
                <>
                  ...
                  <p className="text-sm text-belly-green font-medium">Delivery fee: {formatPeso(deliveryFee)}</p>
                </>
              )}
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    className="flex h-11 w-full rounded-xl border border-belly-brown/20 bg-white px-4 text-sm"
                    value={slotId ?? ''}
          ...
          {!user && (
            <p className="text-sm text-belly-brown/70">
              <Link to="/login" className="text-belly-red underline">Login</Link> to save order history, or continue as guest.
            </p>
          )}
        </div>
        ...
        <Card className="h-fit lg:col-span-2">
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex justify-between font-bold text-lg"><span>Total</span><span className="text-belly-red">{formatPeso(total)}</span></div>
